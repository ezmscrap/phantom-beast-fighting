...
- `@react-three/fiber` と `@react-three/cannon` を用いた物理演算。
- 銀/金ダイスの面には Material Symbols Outlined のアイコン (剣/魔法/チェス) を貼付。
- ダイスが停止後、上向き面を解析し出目を決定。その結果を tallies として UI に反映。

## デバッグモード

`appConfig.diceDebug.enabled` を `true` にすると以下が有効:
- 初期配置をテスト用ユニットに差し替え。
- ダイス設定パネルからサイコロサイズ、生成高さ、インパルスの強さをリアルタイムで変更。
- 表示開始と同時にダイス演出を再生し、出目をデバッグ表示。

## 行動種別

- **通常アクション**: R1 のサイコロでダイスロール。処理後 R2→R1, R3→R2, R1→R3 と回転。
- **作戦アクション**: R1, R2 の2個で判定。両銀のときは次ラウンドで金を R1 に移動。
- **起死回生**: 全サイコロを使用。結果に応じてサイコロ再配置を行う。
