...
通信を行わず、1つのブラウザ内で完結するモードです。物理的なゲームボードがない場合や、動作確認、ログによる振り返りなどに使用できます。
...
* 受け取ったログを自分のブラウザ内のゲームロジックに入力することで、対戦画面を**ミラーリング**します。
* **観戦の連鎖:** 観戦者は、自分が受け取ったデータをさらに「別の観戦者」に**リレー**することができます。これにより、配信者のPC負荷を分散させつつ、多人数での観戦が可能になります。
